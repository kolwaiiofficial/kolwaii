...
import { Web3KawaiiTwitterBot } from "../tools/twitter";
...
export class SolanaAgentKit {
  ...
  private twitterBot?: Web3KawaiiTwitterBot;
  ...
  async tweetAboutNFT(nftAddress: PublicKey, imageUrl?: string): Promise<string> {
    if (!this.twitterBot) {
      throw new Error('Twitter bot not initialized. Please provide Twitter credentials.');
    }
    return this.twitterBot.tweetAboutNFT(nftAddress, imageUrl);
  }
  ...
  async tweetAboutPrice(tokenSymbol: string, price: number): Promise<string> {
    ...
    return this.twitterBot.tweetAboutPrice(tokenSymbol, price);
  }
  ...
  async tweetAboutSolanaUpdate(update: string): Promise<string> {
    ...
    return this.twitterBot.tweetAboutSolanaUpdate(update);
  }
  ...
  async respondToMention(mentionId: string, content: string): Promise<string> {
    ...
    return this.twitterBot.respondToMention(mentionId, content);
  }
}
